...
## 😀 나의 풀이: <br>
-> 내 코드 너무 복잡한 것 같아서, 최대한 간단하게 풀 수 없을까 고민 엄청 한 문제 <br>
-> 또 3개중 1개 정도씩 자꾸 테스트케이스 오류가 나서 계속 코드를 고쳤던 문제 <br>
-> 근데 남들 푼 거보면 다들 엄청 복잡.... <br>
-> 설명: 주석으로 풀이 순서를 먼저 생각해보았다. 일단, 현재위치와 park의 크기를 변수에 담아 저장하였다. 처음엔 장애물도 저장하려고 했는데, 장애물의 갯수는 알 수 없으므로, 그냥 조건문으로 아래서 처리해주기로 하였다. <br>
-> 현재값은 반복문을 돌면서 S를 만나면, now의 값에 저장해 현재 위치를 저장해두었다. <br>
-> 그리고 routes 의 요소에 하나씩 접근하면서, split함수를 사용하여, 방향과 이동 칸 수를 각각 변수에 저장했다. 그리고 방향에 따라 각각 처리를 해주었다. 각 방향의 처리로는 park의 크기를 벗어나면, 그 경로는 패스하므로 coninue 해주었고, 그게아니라면, 방해물을 점검하기 위해 이동할 곳들의 값들을 mons 배열에 넣고, 'X'가 존재하면, 방해물이 있어서 continue를 통해 그 경로를 패스했다. 그렇게 몬스터도 없다면, x나 y값을 update시켜주었다. 그리고 마지막에 now의 값에 대입시켜주었다. <br><br>

##  ✔️ What I learned: <br>
-> 하나의 경로 이동 후, 최종 위치에 방해물 있으면 경로를 패스한다고 생각했다. 그래서 원하는 결괏값이 나오지 않았다. 하지만, 지나가는 경로 내에 방해물이 있어도 패스해야하므로 어려운 문제였다. 
