...
-> 나는 각각 구간을 일일히 집합에 넣고, 중복을 제거하여 그 차이의 길이를 반환하도록 짰다. 하지만 마지막 테스트 케이스에서 원하는 결괏값이 출력되지 않는 것이다.. 문제는 3개가 모두 겹칠 때를 고려해주지 않아서인 것이다!! 그래서 그 방법을 고민 해보았는데,,각각을 다른 집합에 넣고 그것들의 교집합 수를 구하면 어떨까 생각을 하였다. 근데 집합의 요소들이 2차원 리스트 형태여서, 쉽지않다.<br><br>
