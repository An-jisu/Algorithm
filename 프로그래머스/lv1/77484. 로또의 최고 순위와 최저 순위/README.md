...
## ❤️ 문제 핵심: <br>
: 일부가 안보이는 로또 번호 배열의 최고, 최저 순위의 리스트를 반환한다. <br><br>

## 😀 나의 풀이: <br>
![image](https://github.com/An-jisu/Algorithm/assets/70849122/80cbdba0-9923-4981-93dc-61c3124dbebf)  <br>
-> 일치하는 것의 개수와 0의 수에 따라 순위가 달라진다고 생각하여 각각을 구하고, 그에 따른 순위를 반환해주었다. 그런데 완전 효율적인 알고리즘은 아닌 것 같다. 마지막에 등수를 반환할 때, 0이나 count값이 하나도 없거나 6개인 경우의 예외를 처리해주어야 하기 때문에 조건문 분기가 많이 일어났다. 순위와 일치 개수가 거꾸로 되는데, 이 부분을 어떻게 처리할 것인지가 관건인 것 같다. <br><br>

## ⭕ 다른 사람의 풀이: <br>
![image](https://github.com/An-jisu/Algorithm/assets/70849122/7c9bb2b6-377d-4e2e-bc70-3ecf8a8035b1) <br>
-> 나와 같은 풀이 방법이지만, 마지막에 분기 부분을 위와 같이 표현한 것이다. rank 배열을 이용하여, 총 일치하는 수를 배열의 인덱스로 두어, 그에 따른 순위를 그 인덱스의 값으로 둔 것이다!! <br><br>

## ✔️ What I learned: <br> 
-> 숫자를 순차적으로 하나씩 접근하는 것은, 이렇게 리스트를 이용해서 값을 처리할 수 있다는 것!!
